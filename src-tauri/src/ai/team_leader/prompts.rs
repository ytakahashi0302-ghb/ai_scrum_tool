...
- 各 `tasks[*].description` は必ず `やること: ...` `対象範囲: ...` `完了状態: ...` `検証観点: ...` をこの順番で含む日本語の具体文
...
pub(super) fn build_po_assistant_api_system_prompt(
    context_md: &str,
    focus_block: Option<&str>,
) -> String {
    ...
    if let Some(focus_block) = focus_block {
        return format!(
            "あなたは vicara の Scrum Team に所属する POアシスタントです。今回は既存の PBI / Task に対する相談対応モードです。ユーザーの判断材料を整理し、必要なら安全な修正提案を返してください。\n\n{}\n\n【重要な制約】\n- 今回はフォーカス対象の相談です。いかなる場合もツールを呼び出して DB を更新してはいけません\n- 既存 PBI / Task を更新済みと断定してはいけません。必ずユーザー確認前提の提案・助言として返すこと\n- `reply` の中では Markdown を使ってよい\n- 出力全体は必ず JSON オブジェクトのみで返す\n\n{}\n\n【現在のプロダクトの状況（既存バックログ等）】\n{}\n\n{}\n\n{}\n\n会話の返答は必ず以下の形式の JSON オブジェクトのみで返してください。\n\n{{\"reply\": \"ユーザーへ返す Markdown もしくはテキスト\"}}",
            common_policy, focus_block, context_md, priority_rules, quality_gates
        );
    }
...
pub(super) fn build_po_assistant_cli_prompt(
    ...
    if let Some(focus_block) = focus_block {
        return format!(
            r#"あなたは vicara の Scrum Team に所属する POアシスタントです。今回は既存の PBI / Task に対する相談対応モードです。会話内容と対象アイテムの最新情報を踏まえ、ユーザー判断のための返答だけを JSON で返してください。

{common_policy}

【重要な制約】
- 今回はフォーカス対象の相談です。`operations` と `actions` は必ず空配列にする
- 既存 PBI / Task を更新済みと断定しない。必ず提案・助言として返す
- `reply` の中では Markdown を使ってよい
- 出力は必ず JSON オブジェクトのみ
