-- EPIC51: llm_usage_events.source_kind の CHECK 制約に 'retrospective' を追加する。
-- SQLite は CHECK 制約の ALTER をサポートしないため、テーブルをリビルドする。
-- 注: tauri-plugin-sql はマイグレーション自体をトランザクションで実行するため、
-- ここで BEGIN TRANSACTION を書かずに PRAGMA のみを使用する。
...
INSERT INTO llm_usage_events_new SELECT * FROM llm_usage_events;

DROP TABLE llm_usage_events;

ALTER TABLE llm_usage_events_new RENAME TO llm_usage_events;
