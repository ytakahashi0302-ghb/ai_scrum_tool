...
- [ ] **既存タスク編集時のキー入力不備**: アクティブスプリントのボードで既存タスクを編集する際、スペースやエンターキーを押すと背面のタスクカードにフォーカスが移り、文字入力ができなくなる。これはモーダルが `TaskCard` の子要素として定義されているため、キーイベントが `dnd-kit` のリスナーにバブリングしてドラッグ操作のトリガーとして誤検知されることが原因。モーダル内でのイベント伝播停止が必要。
