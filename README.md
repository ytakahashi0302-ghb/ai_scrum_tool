# MicroScrum AI v1.0.0

> **ソロ開発者のための自律AIスクラムツール。複数のAIエージェントがチームとして並行稼働し、あなたの代わりに計画・実装・検証を前へ進めます。**

MicroScrum AI は、スクラムを人間とAIの共通言語に変換し、ソロ開発を「ひとりで全部やる作業」から「AIチームを率いる開発体験」へ変えるローカルファーストなデスクトップアプリです。  
アイデアの壁打ち、プロジェクト文脈の整理、スプリント計画、ロール分担、Claude Code CLI による実装実行までをひとつのUIに統合し、v1.0.0 ではついに複数エージェントの並行稼働と Terminal Dock による可視化を備えたリリース版に到達しました。
...
- **AI Team Leader**: バックログ優先度、進捗、技術相談を横断的に支援するサイドバー型のリードAIを搭載。
- **Inception Deck + Scaffold**: AIとの壁打ちから `PRODUCT_CONTEXT.md`、`ARCHITECTURE.md`、`Rule.md`、`AGENT.md`、`.claude/settings.json` まで一気通貫で整備。
...
MicroScrum AI では、**スクラムを人間とAIの共通言語**として採用しています。
...
MicroScrum AI のマルチエージェント実行と Scaffold の中核は **Claude Code CLI** です。  
...
AI Team Leader、AIによる壁打ち、タスク分解、モデル取得などの体験を安定して使うため、**Anthropic / Gemini の両方を登録しておく構成を推奨**します。
...
MicroScrum AI の Planning は、単なるタスク管理ではありません。  
AI Team Leader や分解フローを通して、ソロ開発者が迷いやすい「何から着手するか」「どこまで切るか」を前倒しで支援します。
...
体験としては、ソロ開発なのに画面の下で複数のAIエンジニアが各自の端末を持って作業している感覚に近く、MicroScrum AI が最もユニークな価値を発揮する瞬間です。

### Step 6. Review & Done

ターミナルの完了状態とログを確認し、成果物をレビューします。

- `Completed` になったタスクを確認する
- ボード上の進捗と依存関係を見直す
- 必要に応じてカンバン上でステータスを調整する
- ストーリー完了後、スプリントを締める

実装そのものはAIに任せつつ、**最終判断と品質の責任は人間が持つ**。この分担が MicroScrum AI の基本姿勢です。
...
| 機能 | 説明 |
|------|------|
| **AI Team Leader** | バックログ優先度、進捗、技術判断を相談できるサイドバー型のリードAI |
...
| | Devin / Claude Code 単体 | MetaGPT | **MicroScrum AI** |
|---|---|---|---|
...
v1.0.0 は完成ではなく、**ソロ開発者のためのAIスクラムチーム**を本格的に育てていく出発点です。
...
アーキテクチャ全体の考え方は [ARCHITECTURE.md](./docs/ARCHITECTURE.md) に整理されています。

---

*MicroScrum AI v1.0.0 - AI-native Scrum for solo builders.*
